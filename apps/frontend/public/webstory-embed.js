;(function () {
  ...
  function renderStoryDirectly(
    ...
    // Track viewport visibility - Views should be when player appears in viewport
    var viewportTracked = false
    var viewportApiBaseUrl = apiBaseUrl || container.getAttribute('data-api-url') || DEFAULT_API_BASE_URL
    if (window.IntersectionObserver) {
      var viewportObserver = new IntersectionObserver(
        function (entries) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting && entry.intersectionRatio >= 0.5 && !viewportTracked) {
              viewportTracked = true
              // Track viewport event - Views = number of times player appears in viewport
              fetch(viewportApiBaseUrl + '/api/analytics/track', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  storyId: storyId,
                  eventType: 'player_viewport',
                  sessionId: 'viewport_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9),
                  metadata: { timestamp: new Date().toISOString() }
                }),
                keepalive: true
              }).catch(function(e) {
                console.debug('Viewport tracking error:', e)
              })
            }
          })
        },
    ...
    setTimeout(function () {
      ...
      fetch(apiBaseUrl + '/api/stories/public/' + storyId + '?ts=' + Date.now())
          ...
          var buttonsHtml = buttonData
            ...
            .map(function (btn) {
              ...
              return (
                ...
                '" target="_blank" rel="noopener noreferrer" class="snappy-frame-link-btn" data-frame-idx="' +
                btn.idx +
                '" onclick="event.stopPropagation(); event.preventDefault(); trackEvent(\'cta_click\',{frameIndex:' +
                btn.idx +
                '}); window.open(\'' +
                escapeAttr(btn.link) +
                "', '_blank', 'noopener,noreferrer');\" style=\"position:absolute;left:" +
        ...
        var slides=document.querySelectorAll('.slide');var idx=0;var timer=null;var story=${JSON.stringify(story)};var frames=${JSON.stringify(frames)};var loop=${!!loop};var defaultDur=story.defaultDurationMs||5000;var frameDurations=(frames||[]).map(function(f){return (f&&typeof f.durationMs==='number'&&f.durationMs>0)?f.durationMs:defaultDur;});var storyId='${storyData.id}';var apiBaseUrl='${apiBaseUrl}';var sessionId='session_'+Date.now()+'_'+Math.random().toString(36).substr(2,9);var frameStartTime=Date.now();var timeSpentTracker=null;function trackEvent(eventType,options){try{var eventData={storyId:storyId,eventType:eventType,sessionId:sessionId,frameIndex:options&&options.frameIndex?options.frameIndex:undefined,value:options&&options.value?options.value:undefined,metadata:{timestamp:new Date().toISOString()}};fetch(apiBaseUrl+'/api/analytics/track',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(eventData),keepalive:true}).catch(function(e){console.debug('Analytics error:',e);});}catch(e){console.debug('Analytics error:',e);}}function animateProgressBar(i){var activeSlide=slides[i];if(!activeSlide)return;var bars=activeSlide.querySelectorAll('.progress-bar');var dur=frameDurations[i]||defaultDur;bars.forEach(function(bar,bidx){var dataIdx=parseInt(bar.getAttribute('data-idx')||bidx);if(dataIdx<i){bar.style.transition='none';bar.style.width='100%';}else if(dataIdx===i){bar.style.transition='none';bar.style.width='0%';bar.style.display='block';setTimeout(function(){bar.style.transition='width '+dur+'ms linear';bar.style.width='100%';},10);}else{bar.style.transition='none';bar.style.width='0%';}});}function show(i){if(timeSpentTracker){clearInterval(timeSpentTracker);}var timeSpent=Date.now()-frameStartTime;if(timeSpent>100){trackEvent('time_spent',{value:timeSpent});}frameStartTime=Date.now();slides.forEach(function(s,j){s.classList.toggle('active',j===i);});var buttons=document.querySelectorAll('.snappy-frame-link-btn');buttons.forEach(function(btn){var btnIdx=parseInt(btn.getAttribute('data-frame-idx')||'-1');btn.style.display=btnIdx===i?'block':'none';});animateProgressBar(i);trackEvent('frame_view',{frameIndex:i});timeSpentTracker=setInterval(function(){var elapsed=Date.now()-frameStartTime;if(elapsed>=1000){trackEvent('time_spent',{value:1000});frameStartTime=Date.now();}},1000);}function next(){trackEvent('navigation_click',{frameIndex:idx,direction:'next'});if(loop){idx=(idx+1)%slides.length;}else{if(idx<slides.length-1){idx++;}else{trackEvent('story_complete',{});return;}}show(idx);if(${!!autoplay}){scheduleNext();}}function prev(){trackEvent('navigation_click',{frameIndex:idx,direction:'prev'});if(loop){idx=(idx-1+slides.length)%slides.length;}else{if(idx>0){idx--;}else{return;}}show(idx);if(${!!autoplay}){scheduleNext();}}document.getElementById('navLeft').onclick=prev;document.getElementById('navRight').onclick=next;function scheduleNext(){if(timer){clearTimeout(timer);}var dur=frameDurations[idx]||defaultDur;timer=setTimeout(function(){if(loop){idx=(idx+1)%slides.length;}else{if(idx<slides.length-1){idx++;}else{trackEvent('story_complete',{});return;}}show(idx);if(loop){scheduleNext();}else if(idx<slides.length-1){scheduleNext();}},dur);}function handleFrameLink(url){try{if(url){window.open(url,'_blank','noopener,noreferrer');}}catch(e){}}show(idx);if(${!!autoplay}){scheduleNext();}
          ...
          // Track viewport visibility - Views should be when player appears in viewport
          var viewportTracked2 = false
          var viewportApiBaseUrl2 = apiBaseUrl || container.getAttribute('data-api-url') || DEFAULT_API_BASE_URL
          if (window.IntersectionObserver) {
            var viewportObserver2 = new IntersectionObserver(
              function (entries) {
                entries.forEach(function (entry) {
                  if (entry.isIntersecting && entry.intersectionRatio >= 0.5 && !viewportTracked2) {
                    viewportTracked2 = true
                    // Track viewport event - Views = number of times player appears in viewport
                    fetch(viewportApiBaseUrl2 + '/api/analytics/track', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
            ...
            viewportObserver2.observe(container)
          }
