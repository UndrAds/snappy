...
import { Eye, Clock, Image, TrendingUp, BarChart3, Info, ArrowLeft } from 'lucide-react'
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export default function UserAnalyticsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [analyticsResponse, storiesResponse] = await Promise.all([
        ...
        adminAPI.getAllStories({ userId, limit: 1000 }),
      ])
  ...
  // Calculate totals
  const totals = userAnalytics?.analytics.reduce(
    (acc, a) => ({
  ...
  const overallAvgPostsSeen =
    userAnalytics && userAnalytics.analytics.length > 0
      ? totals.avgPostsSeen / userAnalytics.analytics.length
      : 0
  const overallAvgTimeSpent =
    userAnalytics && userAnalytics.analytics.length > 0
      ? totals.avgTimeSpent / userAnalytics.analytics.length
      : 0
  const overallAvgAdsSeen =
    userAnalytics && userAnalytics.analytics.length > 0
      ? totals.avgAdsSeen / userAnalytics.analytics.length
      : 0
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Track performance and engagement metrics for this user's stories
            </p>
          </div>
        </div>
          ...
          <CardContent>
            {userAnalytics.analytics.length === 0 ? (
              <div className="py-8 text-center text-muted-foreground">
                ...
                this user's stories are viewed.
              </div>
            ) : (
              <TooltipProvider>
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {userAnalytics.analytics.map((analyticsItem) => {
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-3 font-medium">
                              {analyticsItem.storyTitle || 'Unknown Story'}
                            </td>
